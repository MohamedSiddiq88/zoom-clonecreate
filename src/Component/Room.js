...
const startScreenSharing = () => {
    ...
    .then((mediaStream) => {
      ...
      connectedUsers.forEach((user) => {
        if (user !== peerId) {
          ...
          socket.emit('replace-remote-stream', roomId, user, mediaStream);
        }
      });
  ...
  const call = (remotePeerId) => {
    navigator.mediaDevices.getUserMedia({ video: true, audio: true })
      .then((mediaStream) => {
        const stream = screenSharingUser ? currentUserVideoRef.current.srcObject : mediaStream;
  ...
  return (
	...
	<div className='row'>
      ...
      {screenSharingUser === peerId ? (
            <button className="btn btn-primary sharing-button" onClick={stopScreenSharing}>
              Stop Screen Sharing
            </button>
          ) : (
